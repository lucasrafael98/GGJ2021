...
{	
	...
	/// </summary>
	[AddComponentMenu("TopDown Engine/Environment/Double Key Operated Zone")]
	public class DoubleKeyOperatedZone : ButtonActivated 
	{
		[Header("Key")]
		...
		{
			MoreMountains.InventoryEngine.InventoryItem item = _targetInventory.Content[0];
			if(item == null){
				KeyAction[1].Invoke();
			}
			else{
